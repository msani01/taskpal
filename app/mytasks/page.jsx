...
const page = () => {
  ...
  const handleStatusToggle = async (task) => {
    ...
    if (task.status === "Completed") {
      newStatus = now > dueDate ? "Overdue" : "In Progress";
    } else {
      newStatus = "Completed";
    }
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-gradient-to-r from-gray-50 to-blue-100 relative">
      ...
      >
        <div className="flex flex-col space-y-4">
          <Link href="/" className="mb-5">
            <h1 className="text-2xl font-semibold text-gray-800">TaskPal</h1>
          </Link>
      ...
      <main className="w-full p-4 sm:p-6 flex flex-col space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          {["All", "Pending", "Completed", "In Progress", "Overdue"].map((status) => (
            <button
              key={status}
